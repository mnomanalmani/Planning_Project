...
def obj_func_state(state):
    ...
    state = np.array([*state[:-2], *prev_state.qpos[-2:]])
...
while(1):
    ...
    obj_func_l.append(np.linalg.norm(Dfs(x0)))
    grad_func_l.append(fs(x1))

# plt.plot(obj_func_l)
# plt.show()

# plt.plot(grad_func_l)
# plt.show()

#Now let us define an objective function to minimize the qpos difference achieved
...
for timestep in range(len(qpos_traj)):

    global target_qpos
    target_qpos = qpos_traj[timestep]
    ...
    while(1):
        ...
        grad_func_l.append(np.linalg.norm(Df(x0)))
        obj_func_l.append(f(x0))
        render()
